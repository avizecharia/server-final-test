...
import { allLocations, getFifthByOarg, getFifthByYear, getForthAll, getForthArea, getSixthArea } from "../services/relationships.service"
import { getFirst, getSecAll, getSecCity, getSecContry, getSecRegion, getThirdAll, getThirdBy10Year, getThirdBy5Year, getThirdByYear, getThirdByYearRange } from "../services/analysis.service"
import { IAttack } from "../types/interfaces"
import { createAttack, delAttack, updateAttack } from "../services/crud.service"
import { io } from "../app"
...
export const handelConnection = async (client: Socket) => {
    ...
    client.on('year-range-trend', async (yearStart: string, yearEnd: string) => {
        client.emit('year-range-trend', await getThirdByYearRange(yearStart, yearEnd))
    })
